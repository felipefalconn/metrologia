...
class BlocoPadraoApp:
    def __init__(self, root):
        ...
        self.figure = plt.Figure(figsize=(8, 6), dpi=100)  # Aumentando o tamanho da figura
    ...
    def load_data(self):
        ...
        for c in standard_blocks:
            ...
            block_stdev = 2.365 * pstdev(block_data)
    ...
    def on_block_selected(self, event):
        ...
        if selected_block in self.data:
            ...
            self.text.insert(tk.END, f"Bloco Padrão: {selected_block}\n")
    ...
    def plot_data(self, measures):
        self.ax.clear()
        x_values = list(range(1, len(measures) + 1))  # Criar valores de x correspondentes ao índice das medidas
        self.ax.plot(x_values, measures, marker='o')  # Usar os valores de x e as medidas
        self.ax.set_title("Medidas")
        self.ax.set_xlabel("Índice")
        self.ax.set_ylabel("Medida")

        # Definir o formato dos números no eixo y
        self.ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.3f'))  # Definir 3 casas decimais
